...
use App\Models\Firstyearuploadgrade;
use App\Models\Gradingperiod;

class UploadGradesFirstYearController extends Controller
{
    public function index(Request $request) {
        ...
        ->where('gradeschools.gr_school_id', '=', 1)
        ...
        ->when(
            ...
            // var_dump(simplePaginate(10));
        return view('upload_grades.first_year.index', compact('enrollments'));
        // ->when(
        //     $request->search,
        //     function (Builder $builder) use ($request) {
        //         $builder->where('student_id', 'like', "%{$request->search}%")
        ...
        // $sql = "SELECT gradeschools.gr_school AS gr_school, gradeschools.gr_school_id AS gr_school_id, 
        //         gradeschools.created_at as gr_created_at, gradeschools.updated_at AS gr_updated_at,
        //         enrollmentlabels.enrollment_status_id  as enrollment_status_id, enrollmentlabels.enrollment_status as enrollment_status, 
        //         enrollmentlabels.created_at AS el_created_at, enrollmentlabels.updated_at as el_updated_at, enrollments.*
        //         FROM `enrollments` 
        //         LEFT JOIN `gradeschools`
        //             ON enrollments.grade_school = gradeschools.gr_school_id
        //         LEFT JOIN `enrollmentlabels`
        //             ON enrollmentlabels.enrollment_status_id = enrollments.enrollment_status
        //         WHERE gradeschools.gr_school_id = '1' && enrollmentlabels.enrollment_status_id = '1'";
        
        // $enrollments = DB::select($sql)

        // $enrollments = Enrollment::query()
        // ->join('gradeschools', 'enrollments.grade_school', '=', 'gradeschools.gr_school_id')
        // ->join('enrollmentlabels', 'enrollmentlabels.enrollment_status_id', '=', 'enrollments.enrollment_status')
        // ->where('gradeschools.gr_school_id', '=', 1)
        // ->orwhere('enrollmentlabels.enrollment_status_id', '=', 1)
        // ->get()

        
        // if($request->filled('search_text')) {
        //     $enrollments->where('student_id', 'like', "%{$request->search}%")
        //         ->orWhere('first_name', 'like', "%{$request->search}%")
        //         ->orWhere('last_name', 'like', "%{$request->search}%")
        //         ->orwhere(DB::raw("CONCAT(`enrollments`.`first_name`, ' ', `enrollments`.`last_name`)"), 'like', "%".$request->input('search')."%");
    ...
    public function create($id) {
        ...
        $student_details = DB::table('enrollments')->where('enrollments.student_id', '=', $id)->first();

        return view('upload_grades.first_year.create', compact('student_details'));
    }
    ...
    public function store(Request $request) {
        ...
        $check_upload_grades = DB::select("SELECT * FROM firstyearuploadgrades 
        ...
        if ($check_upload_grades) {
            if ($student_details->student_type == '1') {
                $this->validate($request, [
                    ...
                    'grading_period'      => 'required|unique:firstyearuploadgrades,grading_period',    
            ...
            $upload_grades = Firstyearuploadgrade::create([
            ...
            if ($upload_grades) {
                return redirect()->route('upload_grades.first_year.create', $id)->with('success', $fname .' '. $lname .' '. "succesfully uploaded your grades.");
            } else {
                return redirect()->route('upload_grades.first_year.create', $id)->with('error', $fname .' '. $lname .' '. "error uploading your grades.");
            }
        }else{
            if ($student_details->student_type == '1') {
                $this->validate($request, [
    ...
    public function view($id) {
        ...
        return view('upload_grades.first_year.view', compact('view_grade_students', 'view_student_details'));
    }
    ...
    public function edit($id) {
        ...
        $edit_student_first_grading_grades = Firstyearuploadgrade::query()
            ->where('firstyearuploadgrades.student_id', '=', $id)
            ->where('firstyearuploadgrades.grading_period', '=', 1)
            ->first();
        
        $edit_student_second_grading_grades = Firstyearuploadgrade::query()
            ->where('firstyearuploadgrades.student_id', '=', $id)
            ->where('firstyearuploadgrades.grading_period', '=', 2)
            ->first();

        $edit_student_third_grading_grades = Firstyearuploadgrade::query()
            ->where('firstyearuploadgrades.student_id', '=', $id)
            ->where('firstyearuploadgrades.grading_period', '=', 3)
            ->first();

        $edit_student_fourth_grading_grades = Firstyearuploadgrade::query()
            ->where('firstyearuploadgrades.student_id', '=', $id)
            ->where('firstyearuploadgrades.grading_period', '=', 4)
            ->first();

        $check_upload_grades = Firstyearuploadgrade::query()
            ->where('firstyearuploadgrades.student_id', '=', $id)
            ->where('firstyearuploadgrades.grade_school', '=', 1)
            ->first();

        return view('upload_grades.first_year.edit', compact('student_details', 'edit_student_first_grading_grades', 'edit_student_second_grading_grades', 
    ...
    public function update(Request $request, $id) {
        ...
        if ($check_grading_period == '1') {
            $update_first_grading = Firstyearuploadgrade::findorfail($id);
            ...
            if ($res) {
                return redirect()->route('upload_grades.first_year.edit', $student_id)->with('success',  "1st Grading succesfully updated.");
            } else {
                return redirect()->route('upload_grades.first_year.edit', $student_id)->with('error',  "Error updating on grades.");
            }
        } elseif ($check_grading_period == '2') {
            $update_second_grading = Firstyearuploadgrade::findorfail($id);
            ...
            if ($res) {
                return redirect()->route('upload_grades.first_year.edit', $student_id)->with('success',  "2nd Grading succesfully updated.");
            } else {
                return redirect()->route('upload_grades.first_year.edit', $student_id)->with('error',  "Error updating on grades.");
            }
        } elseif ($check_grading_period == '3') {
            $update_third_grading = Firstyearuploadgrade::findorfail($id);
            ...
            if ($res) {
                return redirect()->route('upload_grades.first_year.edit', $student_id)->with('success',  "3rd Grading succesfully updated.");
            } else {
                return redirect()->route('upload_grades.first_year.edit', $student_id)->with('error',  "Error updating on grades.");
            }
        } else if ($check_grading_period == '4') {
            $update_fourth_grading = Firstyearuploadgrade::findorfail($id);
            ...
            if ($res) {
                return redirect()->route('upload_grades.first_year.edit', $student_id)->with('success',  "4th Grading succesfully updated.");
            } else {
                return redirect()->route('upload_grades.first_year.edit', $student_id)->with('error',  "Error updating on grades.");
            }
        }
    ...
    public function createPDF($id) {
       ...
       $data = [
        ...
        $pdf = Pdf::loadView('upload_grades.first_year.view_pdf', $data);
