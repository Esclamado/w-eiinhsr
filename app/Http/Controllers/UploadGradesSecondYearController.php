...
use App\Models\Secondyearuploadgrade;
use App\Models\Gradeschool;
use App\Models\Gradingperiod;

class UploadGradesSecondYearController extends Controller
{
    public function index(Request $request) {
        ...
        ->where('gradeschools.gr_school_id', '=', 2)
        ...
        return view('upload_grades.second_year.index', compact('enrollments'));
    }
    ...
    public function view($id) {
       ...
       $view_student_details = DB::table('enrollments')->where('enrollments.student_id', '=', $id)->where('enrollments.grade_school', '=', 2)->first();
        ...
        return view('upload_grades.second_year.view', compact('view_grade_students', 'view_student_details'));
    }

    public function create($id) {
        $student_details = DB::table('enrollments')->where('enrollments.student_id', '=', $id)->where('enrollments.grade_school', '=', 2)->first();

        return view('upload_grades.second_year.create', compact('student_details'));
    }
    ...
    public function store(Request $request) {
        ...
        $check_upload_grades = DB::select("SELECT * FROM secondyearuploadgrades 
        ...
        if ($check_upload_grades) {
            if ($student_details->student_type == '1') {
                $this->validate($request, [
                    ...
                    'grading_period'      => 'required|unique:secondyearuploadgrades,grading_period',    
                    ...
                    'advance_biology'     => 'required|numeric',
            ...
            $upload_grades = Secondyearuploadgrade::create([
                ...
                'advance_biology'     =>  $request->input('advance_biology'),
                'statistics'          =>  $request->input('statistics'),
            ]);
    
            if ($upload_grades) {
                return redirect()->route('upload_grades.second_year.create', $id)->with('success', $fname .' '. $lname .' '. "succesfully uploaded your grades.");
            } else {
                return redirect()->route('upload_grades.second_year.create', $id)->with('error', $fname .' '. $lname .' '. "error uploading your grades.");
            }
        } else {
            if ($student_details->student_type == '1') {
                $this->validate($request, [
    ...
    public function edit($id) {
        ...
        $edit_student_first_grading_grades = Secondyearuploadgrade::query()
            ->where('secondyearuploadgrades.student_id', '=', $id)
            ->where('secondyearuploadgrades.grading_period', '=', 1)
            ->first();
        
        $edit_student_second_grading_grades = Secondyearuploadgrade::query()
            ->where('secondyearuploadgrades.student_id', '=', $id)
            ->where('secondyearuploadgrades.grading_period', '=', 2)
            ->first();

        $edit_student_third_grading_grades = Secondyearuploadgrade::query()
            ->where('secondyearuploadgrades.student_id', '=', $id)
            ->where('secondyearuploadgrades.grading_period', '=', 3)
            ->first();

        $edit_student_fourth_grading_grades = Secondyearuploadgrade::query()
            ->where('secondyearuploadgrades.student_id', '=', $id)
            ->where('secondyearuploadgrades.grading_period', '=', 4)
            ->first();

        $check_upload_grades = Secondyearuploadgrade::query()
            ->where('secondyearuploadgrades.student_id', '=', $id)
            ->where('secondyearuploadgrades.grade_school', '=', 2)
            ->first();

        return view('upload_grades.second_year.edit', compact('student_details', 'edit_student_first_grading_grades', 'edit_student_second_grading_grades', 
    ...
    public function update(Request $request, $id) {
        ...
        if ($check_grading_period == '1') {
            $update_first_grading = Secondyearuploadgrade::findorfail($id);
            ...
            $update_first_grading->advance_biology        = $request->input('advance_biology');
            $update_first_grading->statistics             = $request->input('statistics');
            $res = $update_first_grading->update();
    
            if ($res) {
                return redirect()->route('upload_grades.second_year.edit', $student_id)->with('success',  "1st Grading succesfully updated.");
            } else {
                return redirect()->route('upload_grades.second_year.edit', $student_id)->with('error',  "Error updating on grades.");
            }
        } elseif ($check_grading_period == '2') {
            $update_second_grading = Secondyearuploadgrade::findorfail($id);
            ...
            $update_second_grading->advance_biology        = $request->input('advance_biology');
            $update_second_grading->statistics             = $request->input('statistics');
            $res = $update_second_grading->update();
    
            if ($res) {
                return redirect()->route('upload_grades.second_year.edit', $student_id)->with('success',  "2nd Grading succesfully updated.");
            } else {
                return redirect()->route('upload_grades.second_year.edit', $student_id)->with('error',  "Error updating on grades.");
            }
        } elseif ($check_grading_period == '3') {
            $update_third_grading = Secondyearuploadgrade::findorfail($id);
            ...
            $update_third_grading->advance_biology        = $request->input('advance_biology');
            $update_third_grading->statistics             = $request->input('statistics');
            $res = $update_third_grading->update();
    
            if ($res) {
                return redirect()->route('upload_grades.second_year.edit', $student_id)->with('success',  "3rd Grading succesfully updated.");
            } else {
                return redirect()->route('upload_grades.second_year.edit', $student_id)->with('error',  "Error updating on grades.");
            }
        } else if ($check_grading_period == '4') {
            $update_fourth_grading = Secondyearuploadgrade::findorfail($id);
            ...
            $update_fourth_grading->advance_biology        = $request->input('advance_biology');
            $update_fourth_grading->statistics             = $request->input('statistics');
            $res = $update_fourth_grading->update();
    
            if ($res) {
                return redirect()->route('upload_grades.second_year.edit', $student_id)->with('success',  "4th Grading succesfully updated.");
            } else {
                return redirect()->route('upload_grades.second_year.edit', $student_id)->with('error',  "Error updating on grades.");
            }
        }
    ...
    public function createPDF($id) {
       ...
       $data = [
        ...
        $pdf = Pdf::loadView('upload_grades.second_year.view_pdf', $data);
